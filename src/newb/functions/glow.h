...
vec3 nlGlow(sampler2D tex, vec2 uv, vec4 diffuse, float shimmer, float sizeX, float sizeY) {
...
vec3 nlcFakeDepthMap(sampler2D sMat, vec3 diffuse, vec2 texcoord, vec2 lightmap, vec2 offset, vec4 dPd) {
  vec3 shiftTex = vec3(0.0, 0.0, 0.0);
  vec3 result = vec3(0.0, 0.0, 0.0);

  shiftTex -= texture2DGrad(sMat, texcoord - offset, dPd.xy, dPd.zw).rgb;
  shiftTex += texture2DGrad(sMat, texcoord + offset, dPd.xy, dPd.zw).rgb;
